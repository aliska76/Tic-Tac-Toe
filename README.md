...
- Debugging and optimization suggestions. Prompt example: ```game.js already include checkwinner function. I want to move checkWinner, handleMove and isDraw functions to game.js file. Remove it from server``` or ```First player didn't recieve "Wait for the opponent", when second player still not connected```
